...
def mask_altaz_artifacts(
    ...
    mask2d = _radial_falloff_mask(arr.shape[-2:], margin_percent, decay_ratio)

    if arr.ndim == 2:
        masked = arr * mask2d
    elif arr.shape[-2:] == mask2d.shape:
        shape = (1,) * (arr.ndim - 2) + mask2d.shape
        masked = arr * mask2d.reshape(shape)
    elif arr.shape[:2] == mask2d.shape:
        shape = mask2d.shape + (1,) * (arr.ndim - 2)
        masked = arr * mask2d.reshape(shape)
    else:
        raise ValueError(
            f"mask_altaz_artifacts: incompatible image shape {arr.shape} for mask {mask2d.shape}"
        )
    ...
    if fill_value is not None:
        ...
        elif masked.shape[-2:] == mask2d.shape:
            fill_shape = (1,) * (masked.ndim - 2) + mask2d.shape
            masked = np.where(fill_mask.reshape(fill_shape), fill_value, masked)
        else:
            fill_shape = mask2d.shape + (1,) * (masked.ndim - 2)
            masked = np.where(fill_mask.reshape(fill_shape), fill_value, masked)
